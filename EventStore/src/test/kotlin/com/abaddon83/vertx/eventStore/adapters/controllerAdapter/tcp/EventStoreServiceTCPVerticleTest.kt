...
internal class EventStoreServiceTCPVerticleTest{
    companion object {
        ...
        fun beforeAll(context: VertxTestContext) {
            val eventStoreServiceTCPVerticle = EventStoreServiceTCPVerticle(ControllerAdapter(MysqlRepositoryAdapter(),FakeEventStreamAdapter()))
    ...
    fun `publish`(context: VertxTestContext){
        ...
        client.connect(7000,"localhost"){conn ->
            if(conn.failed()){
                ...
                FrameHelper.sendFrame("send",EventStoreServiceTCPVerticle.SERVICE_ADDRESS_PUBLISH, "#backtrack", JsonObject.mapFrom(extendedEvent), socket)
                //context.completeNow()
            }
        }
        ...
        client.connect(7000,"localhost"){conn ->
            if(conn.failed()){
                ...
                FrameHelper.sendFrame(
                    "send",
                    EventStoreServiceTCPVerticle.SERVICE_ADDRESS_QUERY,
                    "#backtrack",
                    JsonObject(mapOf(
