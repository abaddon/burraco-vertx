...
import com.abaddon83.burraco.game.events.game.CardDealtWithDiscardDeck
...
internal class Given_GameDraft_When_DealDiscardDeckCard_Then_exception : KcqrsAggregateTestSpecification<Game>(){
    companion object{
    ...
    override fun `when`(): ICommand<Game> = DealDiscardDeckCard(aggregateId, CARD)
...
internal class Given_GameWaitingDealer_When_DealDiscardDeckCard_Then_event : KcqrsAggregateTestSpecification<Game>(){
    companion object{
        val AGGREGATE_ID = GameIdentity.create()
        val PLAYER_ID1=PlayerIdentity.create()
        val PLAYER_ID2=PlayerIdentity.create()
    ...
    override fun expected(): List<IDomainEvent> = listOf(
        CardDealtWithDiscardDeck.create(aggregateId, CARD)
    )

    override fun expectedException(): Exception? = null
}

internal class Given_GameWaitingDealerWith1CardDiscardDeck_When_DealDiscardDeckCard_Then_exception : KcqrsAggregateTestSpecification<Game>(){
    companion object{
        val AGGREGATE_ID = GameIdentity.create()
        val PLAYER_ID1=PlayerIdentity.create()
        val PLAYER_ID2=PlayerIdentity.create()
