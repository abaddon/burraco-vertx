...
//    }
    ...
    private fun createABurracoTrisWith(rank: Ranks.Rank, numCards: Int): BurracoTris =
        BurracoTris.create(
                cards = allRanksWithJollyCards().filter{ c -> c . rank == rank}.take(numCards)
        )


    private fun createABurracoTrisWithAJollyAnd(rank: Ranks.Rank, numCards: Int): BurracoTris =
        BurracoTris.create(
                cards = allRanksWithJollyCards().filter{ c -> c . rank == rank}.take(numCards - 1).plus(
                    Card(suit = Suits.Jolly, rank = Ranks.Jolly)
                )
        )
    ...
    private fun allRanksWithJollyCards(): List<Card> {
        val cards = listOf(allRanksCards(), addJolly()).flatten()
        assert(cards.size == 54)
        return cards.shuffled()
    }

    private fun allRanksCards(): List<Card> {
    ...
    private fun addJolly(): List<Card> {
