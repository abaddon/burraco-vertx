...
internal class Given_existingDealer_When_DealCardToPlayerWithWrongGamId_Then_exception : KcqrsAggregateTestSpecification<Dealer>(){
    companion object{
        val AGGREGATE_ID = DealerIdentity.create()
        val PLAYER_ID1= PlayerIdentity.create()
        val PLAYER_ID2= PlayerIdentity.create()
    ...
    override fun `when`(): ICommand<Dealer> = DealCardToPlayer(aggregateId, gameIdentity = WRONG_GAME_ID, player = PLAYER_ID1)
...
internal class Given_existingDealer_When_DealCardToPlayerWithWrongPlayerId_Then_exception : KcqrsAggregateTestSpecification<Dealer>(){
    companion object{
        val AGGREGATE_ID = DealerIdentity.create()
        val PLAYER_ID1= PlayerIdentity.create()
        val PLAYER_ID2= PlayerIdentity.create()
    ...
    override fun `when`(): ICommand<Dealer> = DealCardToPlayer(aggregateId, gameIdentity = GAME_ID, player = PLAYER_ID_WRONG)

    override fun expected(): List<IDomainEvent> = listOf(

    )

    override fun expectedException(): Exception? = IllegalArgumentException("Player ${PLAYER_ID_WRONG.valueAsString()} is not part of the game")
}

internal class Given_existingDealerWithPlayerWithMaxCards_When_DealCardToPlayer_Then_exception : KcqrsAggregateTestSpecification<Dealer>(){
    companion object{
        val AGGREGATE_ID = DealerIdentity.create()
        val PLAYER_ID1= PlayerIdentity.create()
        val PLAYER_ID2= PlayerIdentity.create()
    ...
    override fun expectedException(): Exception? = IllegalStateException("The player ${PLAYER_ID1.valueAsString()} has the maximum number of cards (11)")
}